...
public:
    KeyMap(void* param);
    void    ProcessKeyEvent(uint8_t virtKey, DWORD keyData);
    void    SetSendDeadKey(bool sendDeadKeyOpt);
    void    SetUseUnicodeKeysym(bool useUnicodeKeysymOpt);
    void    Reset();
    void    ReleaseAllKeys();

private:
    void    BuildUCS2KS_Map(void);
    void    FlushDeadKey(uint8_t* keyState);
    void    StoreModifier(uint8_t* modifierState, uint8_t* keyState);
    void    SetModifier(uint8_t modifierState, uint8_t* keyState);
    void    ReleaseKey(UINT extVkey);
    void    PressKey(UINT extVkey, uint32_t keysym);
    uint32_t  UCS2X(WCHAR UnicodeChar);

private:
    std::map<WCHAR, uint32_t> ucsLatinExtABMap;
    std::map<WCHAR, uint32_t> ucsGreek_CopticMap;
    std::map<WCHAR, uint32_t> ucsCyrillicMap;
    std::map<WCHAR, uint32_t> ucsArmenianMap;
    std::map<WCHAR, uint32_t> ucsHebrewMap;
    std::map<WCHAR, uint32_t> ucsArabicMap;
    std::map<WCHAR, uint32_t> ucsThaiMap;
    std::map<WCHAR, uint32_t> ucsGeorgianMap;
    std::map<WCHAR, uint32_t> ucsHangulJamoMap;
    std::map<WCHAR, uint32_t> ucsLatinExtAddMap;
    std::map<WCHAR, uint32_t> ucsGenPuncMap;
    std::map<WCHAR, uint32_t> ucsCurrSymMap;
    std::map<WCHAR, uint32_t> ucsCJKSym_PuncMap;
    std::map<WCHAR, uint32_t> ucsKatakanaMap;
    std::map<WCHAR, uint32_t> ucsOthersMap;
    std::map<uint32_t, WCHAR> ksLatinExtMap;
    std::map<uint32_t, WCHAR> ksKatakanaMap;
    std::map<uint32_t, WCHAR> ksArabicMap;
    std::map<uint32_t, WCHAR> ksCyrillicMap;
    std::map<uint32_t, WCHAR> ksGreekMap;
    std::map<uint32_t, WCHAR> ksTechnicalMap;
    std::map<uint32_t, WCHAR> ksSpecialMap;
    std::map<uint32_t, WCHAR> ksPublishingMap;
    std::map<uint32_t, WCHAR> ksHebrewMap;
    std::map<uint32_t, WCHAR> ksThaiMap;
    std::map<uint32_t, WCHAR> ksKoreanMap;
    std::map<uint32_t, WCHAR> ksArmenianMap;
    std::map<uint32_t, WCHAR> ksGeorgianMap;
    std::map<uint32_t, WCHAR> ksAzeriMap;
    std::map<uint32_t, WCHAR> ksVietnameseMap;
    std::map<uint32_t, WCHAR> ksCurrencyMap;
    ClientControlCaptureImpl* process_callback_ = nullptr;
    WCHAR      ucsChar[4];
    uint8_t    KBKeysState[256];
    uint32_t   downKeysym[512];
